...
allowable_features = {
    ...
    #gnn.py使用到这个方法。将RDKit分子对象转换为PyTorch Geometric所需的图数据对象。
    # 提取原子特征（原子类型和手性标签）和键特征（键类型和键方向）。
    # 返回包含节点特征、边索引和边特征的图数据对象。
def mol_to_graph_data_obj_simple(mol):
    ...
    :return: graph data object with the attributes: x, edge_index, edge_attr, rdkit_mol
    """
    # atoms
