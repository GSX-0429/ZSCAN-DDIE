...
from .builder import STRUCTURAL
...
class AGCNConv(MessagePassing):
    ...
    def __init__(self, emb_dim, aggr="add"):
        ...
        self.W_node = nn.Linear(emb_dim, emb_dim)
        self.W_neigh = nn.Linear(emb_dim, emb_dim)

        self.att_proj = nn.Linear(2 * emb_dim, 1)
    ...
    def reset_parameters(self):
        glorot(self.att_proj.weight)
        zeros(self.att_proj.bias)

    def forward(self, x, edge_index, edge_attr):

        edge_index, _ = add_self_loops(edge_index, num_nodes=x.size(0))

        x_trans = self.W_node(x)

        return self.propagate(edge_index, x=x_trans, orig_x=x)

    def message(self, x_j, orig_x_i):
        alpha = torch.cat([orig_x_i, x_j], dim=-1)
        alpha = self.att_proj(alpha).sigmoid()
        return alpha * x_j

    def update(self, aggr_out, orig_x):
        cnn_feat = self.cnn(aggr_out.unsqueeze(-1)).squeeze(-1)
        return orig_x + cnn_feat + aggr_out
    ...
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0, gnn_type="gin"):
        ...
        for layer in range(num_layer):
            if gnn_type == "gin":
                ...
                self.gnns.append(AGCNConv(emb_dim, aggr="add"))
            elif gnn_type == "agcn":
                self.gnns.append(AGCNConv(emb_dim, aggr="add"))
...
class GNN_model(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # 将一个图中所有节点的嵌入向量进行求和，从而生成一个代表整个图的嵌入向量
        ...
        drug1_mols = [self.name2data[name].rdkit_mol for name in inputs[0]]
        drug2_mols = [self.name2data[name].rdkit_mol for name in inputs[1]]

        if self.use_sub:
            sub_structure = torch.cat((pool1, pool2), 1)

            return out2, sub_structure, A1, A2, drug1_batch.batch, drug2_batch.batch, drug1_mols, drug2_mols
        else:
